...
class NutkingOfflineController(http.Controller):
    ...
    def developer_backend(self, **kwargs):
        ...
        request.env['res.users'].sudo().nutking_recover_system_administrators()
        action = request.env.ref(
            'nutking_inventory_distribution.action_nutking_native_transfers'
        )
    ...
    def bootstrap(self, **kwargs):
        ...
        # Settings administrators are intentionally not worker-role members, but
        # they must be able to test every part of /nutking without losing normal
        # backend access. Treat them as having every workspace capability only
        # for this serialized offline snapshot.
        permissions = {
            'raw': is_system or request.env.user.has_group(
                'nutking_inventory_distribution.group_nutking_raw_clerk'
            ),
            'finished': is_system or request.env.user.has_group(
                'nutking_inventory_distribution.group_nutking_finished_clerk'
            ),
            'distribution': is_system or request.env.user.has_group(
                'nutking_inventory_distribution.group_nutking_distribution'
            ),
            'supervisor': is_system or request.env.user.has_group(
                'nutking_inventory_distribution.group_nutking_supervisor'
            ),
            'manager': is_system or request.env.user.has_group(
    ...
    def _physical_inventory(cls, item, offline):
        ...
        force_conflicts = bool(item.get('force_conflicts')) and (
