...
const ROUTE_INFO = {
  ...
  operations: ['Stock Operations', 'Draft, confirm, process, and audit Nut King movements'],
...
function projectedBalances() {
  ...
  for (const item of state.queue.filter((entry) => entry.sync_state !== 'error')) {
    if (item.kind === 'stock_operation') queueStockOperationAdjust(balance, item);
...
function projectedOnHand() {
  ...
  for (const item of state.queue.filter((entry) => entry.sync_state !== 'error')) {
    if (item.kind === 'stock_operation') queueStockOperationAdjust(result, item);
    if (item.kind === 'physical_inventory') {
...
function localOperationRows() {
  return state.queue.filter((item) => ['stock_operation', 'operation_event'].includes(item.kind)).map((item) => ({
    ...
    state: item.sync_state === 'error' ? 'error' : 'pending',
    sync_error: item.sync_error || '',
    lines: item.lines || [],
    process_on_sync: item.process_on_sync,
  }));
}
...
function renderOperations() {
  ...
  const filtered = operations.filter((operation) => {
    if (status !== 'all' && operation.state !== status) return false;
  ...
  $('operations-body').innerHTML = filtered.length ? filtered.map((operation) => {
    ...
    const action = operation.local ? `<button class="nk-button small light local-operation-review" data-uid="${html(operation.external_uid)}">Review</button>` : `<button class="nk-button small light server-operation-review" data-id="${operation.id}">Review</button>`;
    return `<tr><td><strong>${html(operation.name)}</strong>${operation.sync_error ? `<div class="nk-small nk-quantity-low">${html(operation.sync_error)}</div>` : ''}</td><td>${formatDateTime(operation.date)}</td><td>${html(operation.operation_label)}</td><td>${html(party)}</td><td class="number">${formatNumber(operation.quantity)}</td><td>${html(operation.user || '')}</td><td>${badge(operation.state)}</td><td>${action}</td></tr>`;
...
function openScan(operationType = '', preset = {}) {
  ...
  state.scan = { operationType: selected, lines: [], preset };
...
function scanProductCandidate() {
  return findProduct($('scan-product').value, operationProductType(state.scan.operationType));
}
...
function buildScanDraft(processOnSync = false) {
  const operationType = $('scan-operation').value;
  const config = OP_CONFIG[operationType] || {};
  const tripValue = $('scan-trip').value;
  const draft = {
    external_uid: uid('NK-OP'), kind: 'stock_operation', operation_type: operationType,
    ...
    lines: clone(state.scan.lines), process_on_sync: processOnSync,
  ...
  if (!validateScanStock(false)) throw new Error('One or more product quantities exceed projected available stock.');
  return draft;
}

function showScanReview() {
  try {
    const draft = buildScanDraft(false);
...
function renderReview(operation, editableActions = false) {
  ...
  $('review-content').innerHTML = `<div class="nk-review-meta"><div><span>Operation</span><strong>${html(operation.operation_label || OP_LABELS[operation.operation_type] || operation.operation_type)}</strong></div><div><span>Status</span><strong>${html(operation.state || (operation.process_on_sync ? 'Ready to Process' : 'Draft'))}</strong></div><div><span>Date</span><strong>${formatDateTime(operation.date || operation.created_on_device)}</strong></div><div><span>Customer / Supplier</span><strong>${html(operation.partner || party)}</strong></div><div><span>Truck / Trip</span><strong>${html([operation.truck || truck, operation.trip || trip].filter((value) => value && value !== 'Not applicable').join(' · ') || 'Not applicable')}</strong></div><div><span>Reference</span><strong>${html(operation.reference || 'Not set')}</strong></div></div><div class="nk-table-wrap"><table class="nk-table"><thead><tr><th>Product</th><th>Barcode</th><th class="number">Quantity</th><th>Unit</th><th>Batch / Lot</th><th>Expiration</th></tr></thead><tbody>${lines.map((line) => { const product = productById(line.product_id); return `<tr><td><strong>${html(line.product || product?.name || '')}</strong></td><td>${html(line.barcode || product?.barcode || '—')}</td><td class="number">${formatNumber(line.quantity)}</td><td>${html(line.uom || product?.uom || '')}</td><td>${html(line.lot_reference || '—')}</td><td>${html(line.expiration_date || '—')}</td></tr>`; }).join('')}</tbody></table></div>${operation.reason || operation.reason_id ? `<div class="nk-card" style="margin-top:14px"><strong>Movement Reason:</strong> ${html(operation.reason || reasonById(operation.reason_id)?.name || '')}</div>` : ''}${operation.notes ? `<div class="nk-card" style="margin-top:14px"><strong>Notes</strong><div class="nk-small nk-muted" style="margin-top:7px">${html(operation.notes)}</div></div>` : ''}${operation.sync_error ? `<div class="nk-warning-note" style="margin-top:14px">${html(operation.sync_error)}</div>` : ''}`;
  $('save-draft').hidden = !editableActions;
  $('confirm-process').hidden = !editableActions;
  if (!editableActions) {
    const canAct = !operation.local && ['draft', 'confirmed'].includes(operation.state);
    $('review-actions').innerHTML = `<button class="nk-button light" data-close-modal="review-modal">Close</button>${canAct && operation.state === 'draft' ? `<button class="nk-button secondary operation-action" data-id="${operation.id}" data-action="confirm">Confirm</button>` : ''}${canAct ? `<button class="nk-button primary operation-action" data-id="${operation.id}" data-action="process">Process</button>` : ''}${operation.web_url && state.snapshot.permissions?.system ? `<a class="nk-button dark" href="${html(operation.web_url)}">Developer View</a>` : ''}`;
  } else {
    $('review-actions').innerHTML = '<button class="nk-button light" id="review-back">Back</button><button class="nk-button secondary" id="save-draft">Save Draft</button><button class="nk-button primary" id="confirm-process">Confirm &amp; Process</button>';
  }
}

async function saveReviewedDraft(processOnSync) {
  if (!state.review?.draft) return;
  const draft = { ...state.review.draft, process_on_sync: processOnSync };
  await queuePut(draft);
  closeModal('review-modal');
  state.scan.lines = [];
  toast(processOnSync ? 'Confirmed action saved. It will process when Odoo is reachable.' : 'Draft saved safely on this device.', 'success');
  navigate('operations');
...
function openLocalOperation(uidValue) {
  ...
  if (!item) return;
  state.review = { source: 'queue', draft: item };
  renderReview({ ...item, local: true, state: item.sync_state === 'error' ? 'error' : item.process_on_sync ? 'confirmed' : 'draft' }, false);
  $('review-actions').innerHTML = `<button class="nk-button light" data-close-modal="review-modal">Close</button>${!item.process_on_sync ? `<button class="nk-button secondary local-mark-process" data-uid="${html(item.external_uid)}">Confirm &amp; Process</button>` : ''}<button class="nk-button danger queue-delete-modal" data-uid="${html(item.external_uid)}">Delete Draft</button>`;
  openModal('review-modal');
}
...
async function operationAction(operationId, action) {
  ...
  await queuePut({ external_uid: uid('NK-OP-ACTION'), kind: 'operation_event', operation_id: Number(operationId), action, created_on_device: nowIso(), sync_state: 'pending', reference: `Operation ${operationId}` });
  toast('Operation action saved on this device.', 'success');
  closeModal('review-modal');
}
...
function addCountScan() {
  ...
  const product = findProduct($('count-scan').value, type === 'raw' ? 'raw_material' : 'finished_good');
  if (!product) {
...
function bindEvents() {
  ...
  $('scan-operation').addEventListener('change', () => { state.scan.operationType = $('scan-operation').value; state.scan.lines = []; populateScanFields(state.scan.operationType); $('scan-modal-title').textContent = OP_LABELS[state.scan.operationType]; renderScanLines(); });
  $('scan-trip').addEventListener('change', syncTripTruck);
  $('scan-add').addEventListener('click', addScanLine);
  $('scan-product').addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); addScanLine(); } });
  ...
  $('review-actions').addEventListener('click', async (event) => {
    ...
    if (event.target.id === 'save-draft') await saveReviewedDraft(false);
    if (event.target.id === 'confirm-process') await saveReviewedDraft(true);
    ...
    const localProcess = event.target.closest('.local-mark-process'); if (localProcess) { const item = state.queue.find((entry) => entry.external_uid === localProcess.dataset.uid); if (item) { item.process_on_sync = true; item.sync_state = 'pending'; item.sync_error = ''; await queuePut(item); closeModal('review-modal'); if (state.serverOnline) syncQueue(false); } }
    const deleteButton = event.target.closest('.queue-delete-modal'); if (deleteButton) { await queueDelete(deleteButton.dataset.uid); closeModal('review-modal'); toast('Local draft deleted.', 'success'); }
  });
  ...
  $('count-scan').addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); addCountScan(); } });
...
async function init() {
  ...
  if ('serviceWorker' in navigator) {
    try { await navigator.serviceWorker.register('/nutking/sw.js', { scope: '/nutking/' }); } catch (error) { console.warn('Nut King service worker registration failed', error); }
  }
