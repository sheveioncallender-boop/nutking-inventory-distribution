...
function renderReview(operation, editableActions = false) {
  ...
  if (operation.local) {
    ...
    if (operation.web_url && state.snapshot.permissions?.system) buttons.push(`<a class="nk-button light" href="${html(operation.web_url)}">Developer View</a>`);
  }
...
async function init() {
  ...
  if ('serviceWorker' in navigator) {
    try {
      const registration = await navigator.serviceWorker.register('/nutking/sw.js?v=0.4.0', { scope: '/nutking/' });
