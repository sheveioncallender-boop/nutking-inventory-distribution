...
## v0.5.7 Community administrator access and workspace menu fix

- Settings administrators no longer receive a forced Nut King Home Action; `/web` opens Odoo's normal backend.
- Explicit backend ACLs allow the developer administrator to use every Nut King Administration menu without worker-role membership.
- `/nutking/backend` opens native Stock Transfers as a reliable recovery route.
- When a Settings administrator manually opens `/nutking/`, the synchronized workspace grants full testing visibility without adding the administrator to worker groups.
- All offline workspace assets are versioned as v0.5.7 to force a clean cache refresh.
