...
class NutkingDashboard(models.Model):
    ...
    def action_receive_raw(self):
        action = self._action('nutking_inventory_distribution.action_nutking_stock_operation')
        action['context'] = {'default_operation_type': 'raw_receipt'}
        action['views'] = [(False, 'form')]
        return action

    def action_issue_raw(self):
        action = self._action('nutking_inventory_distribution.action_nutking_stock_operation')
        action['context'] = {'default_operation_type': 'raw_issue'}
        action['views'] = [(False, 'form')]
        return action

    def action_add_finished(self):
        action = self._action('nutking_inventory_distribution.action_nutking_stock_operation')
        action['context'] = {'default_operation_type': 'finished_add'}
        action['views'] = [(False, 'form')]
        return action
