...
class NutkingOfflineTransaction(models.Model):
    ...
    state = fields.Selection([('pending', 'Pending'), ('processed', 'Processed'), ('error', 'Error'), ('duplicate', 'Duplicate')], default='pending', required=True, index=True)
    operation_id = fields.Many2one('nutking.stock.operation', readonly=True)
    error_message = fields.Text(readonly=True)

    _external_uid_unique = models.Constraint('unique(external_uid)', 'Offline transaction identifier must be unique.')
