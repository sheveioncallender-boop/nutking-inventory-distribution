...
class StockLocation(models.Model):
    ...
    def create(self, vals_list):
        parent_ids = {
            vals.get('location_id') for vals in vals_list
            if vals.get('location_id') and 'nutking_is_location' not in vals
        }
        parents = {parent.id: parent for parent in self.browse(list(parent_ids)).exists()}
        for vals in vals_list:
            parent = parents.get(vals.get('location_id'))
            if parent and parent.nutking_is_location and 'nutking_is_location' not in vals:
                vals['nutking_is_location'] = True
        return super().create(vals_list)

    def write(self, vals):
        if vals.get('location_id') and 'nutking_is_location' not in vals:
            parent = self.browse(vals['location_id']).exists()
            if parent and parent.nutking_is_location:
                vals = dict(vals, nutking_is_location=True)
...
class StockQuant(models.Model):
    ...
    def _domain_location_id(self):
        ...
        if self.env.context.get('nutking_physical_inventory') and location_id:
            return "[('id', 'child_of', context.get('nutking_physical_location_id')), ('usage', '=', 'internal')]"
