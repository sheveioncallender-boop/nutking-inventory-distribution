...
OPERATION_GROUPS = {
    ...
    'finished_issue': ('nutking_inventory_distribution.group_nutking_finished_clerk',),
    'truck_load': (
...
class NutkingStockOperation(models.Model):
    ...
    external_reference = fields.Char(string='Reference', tracking=True, index=True)
    ...
    def _apply_locations(self):
        ...
        mapping = {
            ...
            'finished_issue': (self._location('nutking_inventory_distribution.location_fg_stock'), self._location('nutking_inventory_distribution.location_fg_issued')),
    ...
    def _check_requirements(self):
        reason_required = {
            ...
            'truck_return', 'customer_return', 'finished_damage', 'finished_issue', 'finished_adjustment',
        }
    ...
    def _check_product_category(self):
        ...
        finished_ops = {'finished_add', 'finished_issue', 'truck_load', 'customer_delivery', 'truck_return', 'customer_return', 'finished_damage', 'finished_adjustment'}
        for operation in self:
    ...
    def _check_available_stock(self):
        internal_source_ops = {
            ...
            'finished_issue', 'truck_load', 'customer_delivery', 'truck_return', 'finished_damage',
        }
    ...
    def action_process(self):
        for operation in self:
            ...
            for move in picking.move_ids:
                # Every rapid-scan line creates its own stock move. Use that
                # move's planned quantity so repeated products with different
                # operational batch references keep their correct quantities.
    ...
    def action_reset_draft(self):
        self._check_role_permission()
        invalid = self.filtered(lambda op: op.state != 'cancelled')
        if invalid:
            raise UserError(_('Only cancelled operations can be returned to draft.'))
        self.state = 'draft'
        return True
...
class NutkingStockOperationLine(models.Model):
    ...
    _operation_product_unique = models.Constraint(
        'unique(operation_id, product_id, lot_reference)',
        'Add each product and batch only once per operation. Combine repeated scans on one line.',
    )
